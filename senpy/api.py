...
def test_import():
    ...
    print("Available functions:", [attr for attr in dir(_senpy) if not attr.startswith('_')])
    return True
...
class AccelerometerData:
    ...
    def __init__(self, timestamps: NDArray[np.int64], x: NDArray[np.float64], 
                 y: NDArray[np.float64], z: NDArray[np.float64]):
...
class SpectrogramResult:
    ...
    def __init__(self, freqs: NDArray[np.float64], times: NDArray[np.float64], 
...
class MotionFeatures:
    ...
    def __init__(self, breathing_rate: NDArray[np.float64], heart_rate: NDArray[np.float64],
                 frequency_sum: NDArray[np.float64], breathing_rate_std: NDArray[np.float64],
                 heart_rate_std: NDArray[np.float64], spectrogram: SpectrogramResult):
...
def resample_accelerometer(timestamps: NDArray[np.int64], 
                          x: NDArray[np.float64], 
                          y: NDArray[np.float64], 
                          z: NDArray[np.float64], 
                          target_fs: float) -> AccelerometerData:
    """
...
def compute_jerk(timestamps: NDArray[np.int64],
                x: NDArray[np.float64],
                y: NDArray[np.float64], 
                z: NDArray[np.float64]) -> Tuple[NDArray[np.int64], NDArray[np.float64]]:
    """
...
def compute_magnitude(x: NDArray[np.float64],
                     y: NDArray[np.float64],
...
def compute_spectrogram(signal: NDArray[np.float64],
                       fs: float,
                       nperseg: int,
                       noverlap: int) -> SpectrogramResult:
    """
...
def compute_motion_features(jerk_signal: NDArray[np.float64],
                          fs: float,
                          ...
                          time_resolution: float = 30.0) -> MotionFeatures:
    """
    ...
    result = _senpy.compute_motion_features(
        jerk_signal, fs, window_size, overlap,
        breathing_rate_min_hz, breathing_rate_max_hz,
        heart_rate_min_hz, heart_rate_max_hz,
...
def gaussian_filter_1d(data: NDArray[np.float64], 
                      sigma: float, 
...
def find_peaks(signal: NDArray[np.float64], 
               prominence_threshold: float) -> NDArray[np.int32]:
    """
...
def rolling_std(data: NDArray[np.float64],
               window_minutes: float,
               seconds_per_window: float = 30.0) -> NDArray[np.float64]:
    """
...
__version__ = "1.0.0"
__all__ = [
    'AccelerometerData', 'SpectrogramResult', 'MotionFeatures',
    'resample_accelerometer', 'compute_jerk', 'compute_magnitude', 
    'compute_spectrogram', 'compute_motion_features',
    'hann_window', 'gaussian_filter_1d', 'find_peaks', 'rolling_std',
    'next_power_of_2', 'compute_median', 'compute_percentile',
    'FrequencyRanges', 'SamplingRates'
]
